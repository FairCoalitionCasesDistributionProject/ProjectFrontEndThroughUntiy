...
public class ResultsController : MonoBehaviour
{
    ...
    {
        float height = -105f;
        int numberOfInstantiation = 0;
        for (int i = 0; i < MainControl.relevantCases.Length; i++)
        {
            if (MainControl.relevantCases[i])
            {
                numberOfInstantiation++;
                GameObject newCaseViewRow = Instantiate(CaseViewRow,transform);
                newCaseViewRow.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                newCaseViewRow.transform.position = new Vector3(0, height*numberOfInstantiation ,0);
            }
        }
    }
}